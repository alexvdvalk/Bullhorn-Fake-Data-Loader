<script lang="ts">
  ...
  $: generateItems(numberOfRecords, fields);

  const generateItems = (num: number, fields: Field5[]) => {
    output = [];
    for (let i = 0; i < num; i++) {
      let obj: any = {};
      fields.forEach((field) => {
        if (field.name === "firstName") {
          ...
          obj[field.name] = faker.name.jobTitle();
